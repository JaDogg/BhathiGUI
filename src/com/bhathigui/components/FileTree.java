...
// NEW
  ...
  public FileTree()
  {
    ...
    DefaultMutableTreeNode top = new DefaultMutableTreeNode(
      new IconData(IconProvider.computerIcon, null, "Computer"));

    DefaultMutableTreeNode node;
    File[] roots = File.listRoots();
    for (int k=0; k<roots.length; k++)
    {
      node = new DefaultMutableTreeNode(new IconData(IconProvider.diskIcon, 
...
//    addWindowListener(wndCloser);
    ...
    public void setM_tree(JTree m_tree) {
        this.m_tree = m_tree;
    }
...
//        continue;
    ...
    for (int i=0; i<v.size(); i++)
    {
      ...
      if(nd.m_file.isDirectory()){
        idata = new IconData(IconProvider.folderIcon,IconProvider.folderIcon, nd);
      }else{
        ImageIcon ico = IconProvider.provide(nd.m_file.getAbsolutePath());
        idata = new IconData(ico,ico, nd);
      }
